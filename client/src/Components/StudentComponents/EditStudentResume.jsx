...
const EditStudentResume = () => {
    const navigate = useNavigate();
    const cvData = useContext(StudentData);
    const [loading, setLoading] = useState(false);
    const CvDetails = useContext(CvData);
    // console.log(CvDetails)
    const {firstname, lastname, Achivement, Address, DOB, EducationData, Hobies, Language, Profile_desc, Skills, curr_profession, email, fresher, gender, noAchivement, number, work_Expirience} = CvDetails
    // const { DOB, email, firstname, gender, lastname, number, _id } = cvData.;
    // console.log(Number(number));
    
    // if(noAchivement === "Fresher"){
  ...
  //  }
    ...
    useEffect(() => {
      ...
      console.log(loading + " " + "from basin home");
    ...
    const [userCvData, setUserCvData] = useState({
      ...
      gender: ugender,
      Address: address,
      curr_profession: curr_profession,
    ...
    //   console.log("checking " + Education);
      ...
      const res = await fetch(`/userCVdetailsEdit/${sid}`, {
        method: "POST",
        headers: {
...
export default EditStudentResume
